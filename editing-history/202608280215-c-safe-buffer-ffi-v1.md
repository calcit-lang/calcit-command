...
- Rejects a null output slot before executing the command, so invalid ABI input cannot trigger a process side effect.
- 在执行命令前拒绝空 output slot，避免无效 ABI 输入触发进程副作用。
